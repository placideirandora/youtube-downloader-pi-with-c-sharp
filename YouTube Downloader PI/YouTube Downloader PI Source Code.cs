...
namespace YouTube_Downloader_PI
{
    public partial class mainForm : Form
    {
        ...
        private void btnDowload_Click(object sender, EventArgs e)
        {
            if (textBoxUrl.Text == "")
            {
                MessageBox.Show("No YouTube video url found. Paste/Type the YouTube video url please!", "Paste/Type The YouTube Video Url.", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else if (textBoxUrl.Text.Contains("www.youtube.com/") || textBoxUrl.Text.Contains("WWW.YOUTUBE.COM/") 
                || textBoxUrl.Text.Contains("youtube.com/") || textBoxUrl.Text.Contains("YOUTUBE.COM/")
                || textBoxUrl.Text.Contains("youtu.be/") || textBoxUrl.Text.Contains("YOUTU.BE/")
                || textBoxUrl.Text.Contains("www.youtu.be/") || textBoxUrl.Text.Contains("WWW.YOUTU.BE/"))
            {
                if (textBoxPath.Text != "")
                {
                    statusPanel1.BorderStyle = StatusBarPanelBorderStyle.Sunken;
                    ...
                    try
                    {
                        ...
                        statusPanel1.BorderStyle = StatusBarPanelBorderStyle.Sunken;
                        statusPanel1.Text = "Status: Downloading: " + video.Title + video.VideoExtension ;
                        statusPanel1.ToolTipText = "Current Activity";
                        statusPanel1.AutoSize = StatusBarPanelAutoSize.Spring;
                    }
                    ...
                    {
                        DialogResult Response = MessageBox.Show("Connection failed. Check the connection settings and try again!", "Connection Failed", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                {
                    MessageBox.Show("No storage location found. Select where the video will be saved please!", "Video Storage Location", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            else
            {
                MessageBox.Show("Invalid YouTube video url. Paste/Type the right YouTube video url please!", "Paste/Type The Right YouTube Video Url.", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                textBoxUrl.Text = "";
            }
        }
        ...
        private void linkLabelHelpUrl_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            MessageBox.Show("Go to www.youtube.com, then open/watch the video that you want to download" +
                ...
                " Lastly, After having copied the video url/link, paste it here for being able to download the video.","Copy And Paste The YouTube Video Url" ,MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        {
            MessageBox.Show("Select the storage location/path where the video " +
